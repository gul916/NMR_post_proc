...
def param_proc(data):
    # Asked to the user
    firstDec = data.firstDec
    fullEcho = data.fullEcho
    nbEcho = data.nbEcho
    
    # From Topspin interface
    td = data.td                    # nb de pts complexes  ( td == td/2 )
    dw = data.dw                    # Dwell time between two points
    de = data.de                    # temps de non-acquisition au début
    
    # Calculated
    halfEcho = data.halfEcho
    nbHalfEcho = data.nbHalfEcho
    nbFullEchoTotal = int((nbHalfEcho+1)/2)
    td2 = data.td2                    # nb de pts complexes  ( td == td/2 )
    dw2 = data.dw2
    acquiT = data.acquiT    # temps acquisition total : acquiT = (td-1)*dw2
    
    lb = 3/(np.pi*halfEcho)                # line broadening (Herz)
    dureeSignal = data.dureeSignal
    nbPtHalfEcho = data.nbPtHalfEcho
    nbPtFullEcho = 2*nbPtHalfEcho
    nbPtSignal = data.nbPtSignal
    missingPts = data.missingPts
    nbPtDeadTime = data.nbPtDeadTime
...
def signal_processing(data):
    ...
    zerosToAdd = np.zeros((data.nbPtDeadTime), dtype=np.complex)
    A = np.concatenate((zerosToAdd[:],A[:data.td2-data.nbPtDeadTime]))
#    print("A.size =", A.size)

    # on supprime les points en trop
    echos1D = A[:data.nbPtSignal]
...
#    desc = data.firstDec
#
#    for i in range (0, data.nbHalfEcho):
#        if (desc==True):
#            timei = np.linspace(0,data.halfEcho-data.dw2,data.nbPtHalfEcho)
#        else:
#            timei = np.linspace(-data.halfEcho,-data.dw2,data.nbPtHalfEcho)
#        echos1D[i*data.nbPtHalfEcho:(i+1)*data.nbPtHalfEcho] *= \
#            np.exp(-abs(timei[:])*np.pi*lb)
#        desc = not(desc)
